...
// list of trains
 app.get('/trains', (req:Request,res:Response)=>{
    res.send(data.trains);
})

//list of passengers
 app.get('/passengers', (req:Request,res:Response)=>{
    res.send(data.passengers);
})

// specific train based on train id 
app.get('/train/:id', function(req, res){
    let train = data.trains.find((t: { id: number; }) => t.id === parseInt(req.params.id));
    res.send(train);
    if (!train){
...
app.get('/train/:from/:to', (req, res) => {
    let fromPlace = data.trains.find( (f: { from: string; to: string; }) => f.from === req.params.from && f.to === req.params.to);
    res.send(fromPlace);
});

// updated the train_id the passenger has booked
app.put('/passengers/:id', (req, res) => {
    let passenger = data.passengers.find((t : {id : number})=> t.id === parseInt(req.params.id));
    if (!passenger) {
